...
class ActionReader:
    ...
    def scheduling_action(self, action, RAR_action = False, I_tbs = -1, N_rep = -1):
        '''
        ...
        if I_tbs > -1:
            I_mcs = Itbs_to_Imcs[I_tbs] # users in HARQ will use previous I_tbs

        if N_rep < 0:
            N_rep = N_rep_list[I_rep] # and the same number of repetitions
